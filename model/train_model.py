...
CSV_IN  = str(ROOT / "export/csvs/features_sauheid_v2.csv")
...
# Fill NaN features with column median (keeps 2021 flood rows)
# swvl1 only covers 2023-2025 — fill 2021 with median value
df_work = df[FEATURE_COLS + TARGET_COLS].copy()
# Only dropna on core features; fill the rest with median
core_cols = [c for c in FEATURE_COLS if not any(
    x in c for x in ["swvl1","P_era5","ndvi"])]
df_clean = df_work.dropna(subset=core_cols + TARGET_COLS)
# Fill remaining NaNs with column medians
for col in FEATURE_COLS:
    if df_clean[col].isna().any():
        med = df_clean[col].median()
        # If median is NaN (column entirely empty), fill with 0
...
for horizon, target in [("t+1d", "H_t1"), ("t+2d", "H_t2"), ("t+3d", "H_t3")]:
    ...
    # Linear regression
    # Fill any remaining NaNs with training median before scaling
    X_tr_med = X_tr.median()
    X_tr_f   = X_tr.fillna(X_tr_med)
    X_te_f   = X_te.fillna(X_tr_med)
    X_fl_f   = X_fl.fillna(X_tr_med)

    scaler = StandardScaler()
    lr = LinearRegression()
    lr.fit(scaler.fit_transform(X_tr_f), y_train)
    lr_pred_test  = lr.predict(scaler.transform(X_te_f))
    lr_pred_flood = lr.predict(scaler.transform(X_fl_f))
    ...
    rf_pred_test  = rf.predict(X_te_f)
    rf_pred_flood = rf.predict(X_fl_f)
    ...
    rf_d.fit(X_tr_f, y_tr_d)
    # Convert delta predictions back to absolute H
    rf_d_pred_test  = test.loc[te_idx, "H"].values  + rf_d.predict(X_te_f)
    rf_d_pred_flood = flood.loc[fl_idx, "H"].values + rf_d.predict(X_fl_f)
...
print(f"  Test predictions → export/csvs/predictions_test_2025.csv")
...
print(f"  Flood predictions → export/csvs/predictions_flood_2021.csv")
